function _fillData(box){
	var _this = this,
		...
		tmp = {
				...
				kbox : '<ul>'
					+ '<li>日期：<i><%=totime(obj.date,"y-m-d")%></i></li>'
					+ '<li>时间：<i><%=totime(obj.date,"h:i:s")%></i></li>'
					...
					+ '<li>涨幅：<i class="<%if(obj.rate > 0){%>red<%}else if(obj.rate < 0){%>green<%}%>"><%=cutFixed(obj.rate,3)%>%</i></li>'
					...
					if(k === 'k'){
						stockChart['kBox'] && (stockChart.kBox.innerHTML = rentmpl(
							tmp['kbox'],
							{
								...
								obj : data.data[i],
								cfg : stockChart.options,
								tl : stockChart.options.cycMap[stockChart.options.user.cyc],
								outRule : outRule
							}
						));
					}
