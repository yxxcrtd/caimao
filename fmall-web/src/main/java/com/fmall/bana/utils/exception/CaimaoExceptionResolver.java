...
import com.fmall.bana.utils.JacksonUtils;
...
public class CaimaoExceptionResolver extends SimpleMappingExceptionResolver {
    ...
    private BaseRuntimeException exceptionWapper2(Exception exception) {
        if (exception instanceof CustomerException) {
            ...
            ex.setCode(((CustomerException) exception).getCode().toString());
            return ex;
        }
    ...
    private void convertErrorMessage(HttpServletRequest request, BaseRuntimeException ex) {
        ...
        if ((isNotDataValidationException(ex).booleanValue()) &&
            ...
            for (Iterator localIterator = ex.getExceptions().iterator(); localIterator.hasNext(); ) {
                Object er = localIterator.next();
    ...
    protected String determineViewName(Exception ex, HttpServletRequest request) {
        ...
        if (this.exceptionMappings != null) {
            ...
            if (viewName != null) {
                try {
                    viewName = viewName + "?return=" + URLEncoder.encode(String.valueOf(request.getRequestURL()), "UTF-8");
