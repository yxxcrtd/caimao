...
$(function() {
    ...
    // P2P产品配置的地址
    container.p2pProdUrl = basePath + "/p2p/product/config";

    container.p2pConfigs = [];
    ...
    // 获取产品的配置
    $.commAjax({
        url : container.p2pProdUrl,
        type : "get",
        data : {
            product_id : proMonthId
        },
        success : function(data2) {
            if (data2.success == true) {
                for (i = 0; i < data2.data.productConfig.length; i++) {
                    container.p2pConfigs[data2.data.productConfig[i]['prodLever']] = data2.data.productConfig[i];
                }
            }
        }
    });
    ...
    function createValibleHtml(){
        ...
        for (var i = container.min_ration; i <= container.max_ration; i++) {
            ...
            if (typeof container.p2pConfigs[i] !== "undefined") {
                if (container.p2pConfigs[i]['isAvailable'] == true) {
                    continue;
                }
            }
