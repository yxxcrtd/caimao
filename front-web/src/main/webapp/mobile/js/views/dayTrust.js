...
// 查询当日成交记录
container.curentrustUrl = basePath +  "/stock/child/curentrust";
...
function showCurentrusts(){
	$.commAjax({
		url:container.curentrustUrl,
		data:{
...
function createTrustHtml(trusts){
	...
	var lHtml = '',rHtml='';
	for(var i in trusts){
		lHtml += '<tr id="lTrust'+i+'" class="lTrust">'+
				'<td class="tb-left">'+
		...
		while(trusts[i].entrustTime.length<6){
			trusts[i].entrustTime = "0"+trusts[i].entrustTime;
		}
		
		rHtml += '<tr id="rTrust'+i+'"  class="rTrust">'+
