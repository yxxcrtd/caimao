...
// 获取可买数量 post
container.buyquantity = basePath + "/stock/buyquantity";
// 获取正确代码 get
...
// 购买接口 post
container.buyUrl = basePath+"/stock/buy";
		...
		container.needAmountAjax = $.commAjax({
			...
			success:function(data){
				if(data.success){
					...
					$("#cBuyNumber").html(data.data.curNumber);
					container.curNumber = data.data.curNumber;
				}
			}
...
function choseStockCode(stockCode,stockName){
	...
	if($("#stockCode").val() != container.oldStockCode||$("#cBuyNumber").html()==""){
...
$(function(){
	...
	$.commAjax({
		...
		success:function(data){
			...
			$("#buyBtn").click(function(){
				...
				if(buyNumber < 1 || buyNumber>1000000){
	...
	amouts.click(function(){
		...
		if(container.curNumber){
			$("#buyNumber").val(parseInt(container.curNumber/amoutArr[amouts.index($(this))]));
		}
	});
});
...
function buyAction(){
	var dialog = $.dialog();
	$.commAjax({
		url:container.buyUrl,
		data:{
			homs_combine_id:container.homsData.homsCombineId,
			homs_fund_account:container.homsData.homsFundAccount,
			stock_code:$("#stockCode").val(),
			entrust_quantity:$("#buyNumber").val(),
		...
		success:function(data){
			...
			if(data.success){
				$.dialog({
					...
					title:"ok",
					ok:function(){
						window.location.href = mobileUrl + "/trade/buy.htm";
					}
				})
			}else{
				$.dialog({
