define([
    ...
    'dojo/dom-style',
    'dojo/_base/config',
    'dojo/text!./templates/SideMenu.html',
    'app/common/Data',
    'dojo/when'
], function(declare, _WidgetBase, _TemplatedMixin, ViewMixin, on, dom, domConstruct, query, Fx, domClass, domStyle, cfg, template, Data, when){
    return declare([_WidgetBase, _TemplatedMixin, ViewMixin], {
        ...
        render: function() {
            ...
            var b_base_url = this.baseUrl;
            // 推广开启，并且制定配置中的USER_ID才显示推广链接
            when(Data.getUser(), function (user) {
                // && cfg.popularizeUID.indexOf(user.userId.toString()) < 0
                if (user && cfg.isPopularize) {
                    domConstruct.place(domConstruct.toDom('<li><i class="fa fa-users d-icon">' +
                    '</i><em><a href="'+b_base_url+'/popularize/popularize.htm">推广赚钱</a></em></li>'), me.popularizeNode, 'replace');
                } else {
