define([ 
	...
	return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
        ...
        getBank: function() {
            return this.get('banks')[this.get('bankIndex')];
        },
		...
		_setBanksAttr: function(value) {
			...
			for (; i <len; i++) {
				var html = '<li class="list-chooseBank-item" style="position: relative;">' +
					...
					'<a class="ui-logo-small">' + ((value[i]['isQuickPay'] == '1') ? fastHtml : '') +
					'<img src="' + Config.baseUrl + '/app/resources/image/bosheng/borkers-logo/bank-'+value[i]['bankNo']+'.jpg" width="110" height="25"></a></li>';
		...
		addListeners: function() {
			...
			on(me.bankcardNode, 'mouseenter', function() {
				if (domClass.contains(this, 'dijitTextBoxError')) {
					Tooltip.show('请选择银行', this, 'warning');
				}
			});
		},
		...
		checkValidity: function() {
			...
			if (me.bankIndex === null) {
				...
				setTimeout(function() {
					Tooltip.show('请选择银行', me.bankcardNode, 'warning');
				}, 100);
				return false;
			}
			return true;
