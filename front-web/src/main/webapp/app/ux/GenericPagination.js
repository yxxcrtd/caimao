define([
	...
	"dojox/html/metrics",
	"dojo/i18n!dojox/grid/enhanced/nls/Pagination",
	"app/ux/GenericSelect",
	"app/ux/GenericGridButton"
], function(kernel, declare, array, connect, lang, html, event, query,
	string, keys, template, Dialog, layers, _Plugin, EnhancedGrid,
	Button, NumberTextBox, dijitFocus, _Widget, _TemplatedMixin, _WidgetsInTemplateMixin, metrics, nls, Select, GenericButton){
...
var _ForcedPageStoreLayer = declare("dojox.grid.enhanced.plugins._ForcedPageStoreLayer", layers._StoreLayer, {
	...
	_fetch: function(request){
		...
		request.start = (plugin._currentPage - 1) * (plugin._currentPageSize || 0) + request.start;
...
var _Paginator = declare("dojox.grid.enhanced.plugins._Paginator", [_Widget, _TemplatedMixin], {
	...
	_updateSizeSwitch: function(){
		...
		html.style(this.sizeSwitchNode, "display", this.sizeSwitch ? "" : "none");
		if(!this.sizeSwitch){
			return;
		}
		if(this.sizeSwitchNode.childNodes.length < 1){
	...
	_createSizeSwitchNodes: function() {
		...
		array.forEach(this.pageSizes, function(size){
			...
			options.push({label: value+'', value: size});
			// for accessibility
		...
		select = new Select({name: 'sizeSwitch', options: options, style: 'width: 50px; height: 25px;'});
		select.on('change', lang.hitch(this.plugin, function(size) {
			this.currentPageSize(size);
		}));
		html.create('span', {innerHTML: '每页显示'}, this.sizeSwitchNode, 'first');
		select.placeAt(this.sizeSwitchNode);
		html.create('span', {innerHTML: '条'}, this.sizeSwitchNode, 'last');
	},
...
//		}, this);
	...
	_updatePageStepper: function(){
		...
		html.style(this.pageStepperNode, "display", this.pageStepper ? "" : "none");
		if(!this.pageStepper){
			return;
		}
		if(this.pageStepperNode.childNodes.length < 1){
...
//		createWardBtn("lastPage", nls[9], "last");
		...
		var prevBtn = new GenericButton({text: '上一页', action: 'prev'});
		this.plugin.connect(prevBtn.domNode, "onclick", this.plugin.prevPage);
		var nextBtn = new GenericButton({text: '下一页', action: 'next'});
		this.plugin.connect(nextBtn.domNode, "onclick", this.plugin.nextPage);
		
		prevBtn.placeAt(this.pageStepperNode);
		nextBtn.placeAt(this.pageStepperNode);
	},
...
//		}
	...
	_updatePageStepNodesStyle: function(){
		...
		var value = null,
			...
			pageCount = this.plugin.getTotalPageNum() || 1;
...
//		}, this);
		...
		var preBtn = this.pageStepperNode.childNodes[0];
		var nextBtn = this.pageStepperNode.childNodes[1];
		html.removeClass(preBtn, 'genericButtonDisabled');
		html.removeClass(nextBtn, 'genericButtonDisabled');
		if (curPage === 1) {
			html.addClass(preBtn, 'genericButtonDisabled');
		} 
		if (curPage === pageCount) {
			html.addClass(nextBtn, 'genericButtonDisabled');
		} 
	},
