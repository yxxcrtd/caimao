...
/**
 * Created by WangXu on 2015/5/20.
 */
public class IsMobileDevice extends HandlerInterceptorAdapter {

    private static final Logger logger = LoggerFactory.getLogger(IsMobileDevice.class);
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        //this.logger.info("浏览器标示：" + userAgent);
        ...
        //判断是否有邀请ID
        String inviteUrl = "";
        String inviteId = request.getParameter("i");
        if(inviteId != null && !"".equals(inviteId)){
            inviteUrl = "?i=" + inviteId;
        }
        ...
        if (cookies != null) {
            for (int i = 0; i < cookies.length; i++) {
                Cookie c = cookies[i];
                if (c.getName().equalsIgnoreCase("visit_device")) {
                    if (c.getValue().equals("pc")) {
                        //this.logger.info("我要访问电脑版：" + mobileFlag);
                        return true;
                    } else if (c.getValue().equals("mobile")) {
                        //this.logger.info("我要访问手机版：" + mobileFlag);
                        if("".equals(inviteUrl)){
                            response.sendRedirect("/mobile" + inviteUrl);
                        }else{
                            response.sendRedirect("/mobile/user/login.htm" + inviteUrl);
                        }
                        return false;
                    }
                }
        ...
        //this.logger.info("是否是手机：" + mobileFlag);
