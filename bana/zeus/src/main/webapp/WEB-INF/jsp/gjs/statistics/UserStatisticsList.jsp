...
<div class="body_container">
    ...
    <div class="main_content">
        <div class="container">
            <h4 class="heading">用户基本信息统计</h4>
            <jsp:include page="/WEB-INF/jsp/include/tpl_menu_page.jsp"/>
            <form method="post" class="form-inline" action="/gjs/statistics/user" name="searchForm">
                ...
                <div class="form-group" style="margin-bottom: 5px;">
                    <label>交易所:</label>
                    <select name="exchange" class="form-control input-sm" id="exchange">
                        <option value="">全部</option>
                        <option value="njs" <c:if test="${'njs' == list.exchange}">selected="selected"</c:if>>南交所</option>
                        <option value="sjs" <c:if test="${'sjs' == list.exchange}">selected="selected"</c:if>>上金所</option>
                    </select>
                </div>
                <div class="form-group" style="margin-bottom: 5px;">
                    <label>状态:</label>
                    <select name="status1" class="form-control input-sm" id="status">
                        ...
                        <c:if test="${'' == list.exchange || null == list.exchange}">
            ...
            <div style="padding: 15px 0;">共有<b> ${list.totalCount} </b>条记录</div>
            <div style="padding: 5px 0 15px;">累计注册用户数：<b>${userTotalCount}</b></div>
            ...
            <table class="table table-hover table-bordered" style="width: 1000px;">
                ...
                <c:forEach items="${list.getItems()}" var="u">
                    <tr>
                        ...
                        <td>${u.registerIp}</td>
                        <td><fmt:formatDate value="${u.lastLoginDatetime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
                        <td>${u.lastLoginIp}</td>
                    </tr>
                </c:forEach>
