 ...
 *
 */
public class MemoryManagerByte implements MemoryObjInface{
...
//			ByteBuffer buf_6 = newMomoryManager.allocat(22);
	...
	private MemoryManagerByte momoryManagerByte = null;
	 ...
	 */
	public MemoryManagerByte(int byteSize,int defaultSize,int dilatancySize,boolean isDirect){
		byte_size = byteSize;
	...
	public ByteBuffer allocat() {
		return this.allocat(defaultSize);
	}
	...
	public ByteBuffer allocat(int size) {
		...
		if(byteBuffer == null){
			//如果未获取到在到子内存管理对象里面获取
			MemoryManagerByte nextMomoryManagerByte = this.getMomoryManagerByte();
			...
			//如果没有子内存管理对象就创建一个并获取
			nextMomoryManagerByte = new MemoryManagerByte(byte_size,this.defaultSize,this.dilatancy_size,this.isDirect);
...
//			this.getBufferSet().add(new MomoryBuffer(slicebuf,slicebuf.arrayOffset(),slicebuf.arrayOffset() + slicebuf.limit()));
	 ...
	 */
	private void setMomoryManagerByte(MemoryManagerByte momoryManagerByte) {
	 ...
	 * @return
	 */
	private MemoryManagerByte getMomoryManagerByte() {
