...
import com.caimao.jserver.socket.core.ClosedSessionException;
import com.caimao.jserver.socket.core.CloseingSessionException;
...
public class IoSessionImpl implements IoSession {
	...
	/** 写队列 */
	private ConcurrentLinkedQueue<IoFuture> writeQueue = new ConcurrentLinkedQueue<IoFuture>();
	...
	protected IoSessionImpl(SocketChannel channel, IoReadWriteMachine dispatcher) {
		...
		checkOverTime = new OverTimeCheckTask();
		// this.inBuffer =
		// this.ownerDispatcher.getMemoryManager().allocat(10240);
		// //分配10的输入cache
	}
	...
	void allocatInBuffer() {
		...
		// ByteBuffer buffer =
		// this.ownerDispatcher.getMemoryManager().allocat();
		this.inBuffer = this.ownerDispatcher.getMemoryManager().allocat();

		// TODO:测试打印
		// System.out.println("AllocatThreadName = "
		// + this.ownerDispatcher.threadName);
		// System.out.println("AllocatNum = " +
		// this.ownerDispatcher.getMemoryManager().getBufferNum());
	...
	@Override
	// 关闭会话，关闭会话是异步关闭，如果需要同步，调用IoFuture上的wait
	...
	public void readNow() throws IOException {
		// 如果是主动读，就不再事件触发
		/*if (ownerAcceptor.getConfigure().isOnlyRead()) {
			return;
		}*/
		...
		if (readlen > 0) {
			...
			if (ownerAcceptor.getConfigure().getProtocolHandler() == null) {
				...
				readBuf.flip();
				Message msg = ownerAcceptor.getConfigure().getProtocolHandler().onData(readBuf, this);
				// 处理已经读过的buffer
				...
				if (msg != null) {
					// 如果得到错误消息，应该立即终止读取，并关闭Session
					if (msg.equals(Message.ERROR_MSG)) { // 协议解析发生不可恢复错误
						this.close(); // 发出关闭请求
	...
	public void closeNow0() throws IOException {
		// TODO:测试代码
	...
	private Message readMsg() throws IOException {
		int readlen;
		readlen = this.channel.read(this.inBuffer);
	...
	@Override
	/** 写消息 */
	public IoFuture write(Message msg) {
		if (msg == null) {
			throw new IllegalArgumentException("msg is null");
		}

		WriteFuture future = new WriteFuture(this, ByteBuffer.wrap(msg.getBytes()));
	...
	private class OverTimeCheckTask extends TimerTask {
		...
		public void run() {
			...
			if ((currTime - lastAccessTime) > bothOverTime || (currTime - lastReadTime) > readOverTime
	...
	public IoFuture connect() {
		throw new UnsupportedOperationException("不能进行操作....");
	}
