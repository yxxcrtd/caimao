...
import com.caimao.jserver.socket.core.IoService;
...
public abstract class AbstractIoService implements IoService, DispatcherEventlListener{
	...
	@Override
	public IoProtocol buildProtocolGroup(int prefixByteNum, IoProtocol... handlers) {
	...
	public void onRegisterSession(IoSessionImpl session) {
		...
		IoHandler handler=this.getConfigure().getIoHandler();
		if(handler!=null){
			handler.sessionRegistered(session);
		}
	}
