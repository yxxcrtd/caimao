...
public class NJSDataHandleThread  implements Runnable{
	...
	@Value("${njs.hq.time.no.begin}")
	protected String njsHQTimeNoBegin;

	@Value("${njs.hq.time.no.end}")
	protected String njsHQTimeNoEnd;
	...
	public  Boolean isTradeTime(String  time){
		...
		String strEnd0900="";
		if(!StringUtils.isBlank(njsHQTimeNoEnd)){
			strEnd0900=time.substring(0,8)+njsHQTimeNoEnd;
		}else{
			strEnd0900=time.substring(0,8)+"090000";
		}

		String strBegin0600="";
		if(!StringUtils.isBlank(njsHQTimeNoBegin)){
			strBegin0600=time.substring(0,8)+njsHQTimeNoBegin;
		}else{
			strBegin0600=time.substring(0,8)+"060100";
		}
	...
	private void execute(Snapshot snapshot) throws Exception {
		...
		try{
			if(isTradeTime(snapCandle.getMinTime())){
				...
				logger.info("南交所非交易时间段数据：不生成K线,如果需要重新设置行情时间，可以在application.properties 设置 " + snapshot.toString());
			}
		}catch (Exception ex){
