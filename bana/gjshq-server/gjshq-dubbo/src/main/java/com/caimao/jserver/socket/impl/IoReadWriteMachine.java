...
import com.caimao.jserver.socket.expand.ClientIoSession;
import com.caimao.jserver.socket.expand.ConnectFuture;
 ...
 *
 */
public class IoReadWriteMachine implements Runnable {
	...
	public void init(int maxIoSessionNum) throws Exception {
		...
		memManager = new MemoryManagerByte(IoSession.INBUFFER_SIZE * 100, IoSession.INBUFFER_SIZE,
				IoSession.INBUFFER_SIZE * 100, true);
	 ...
	 *//*
		 ...
		 * 
		 * waitRemoveQueue.offer(closeFuture); this.wakeup(); }
		 */
	...
	private void registerIoSessionNow(IoSessionImpl session) throws IOException {
		...
		/*
		 * if(channel.isRegistered()) { throw new RuntimeException("此通道已经主册过");
		 * }
		 */
	...
	/** 移除并关闭所有在移除队列中的会话 *//*
							 * private void removeIoSessionsNow() { for(;;) {
	...
	private void removeIoSessionNow(CloseFuture future) {
		...
		if (future.isCannel()) {
			...
			try {
				...
				session.getOwnerAcceptor().getConfigure().getIoHandler().sessionClosed(future);
			}
		}
	}
	 ...
	 * 循环处理，//TODO:正在考虑是否按写的字节数来做为判断是在一次循环中写入小于多少的数据进入 下次循环，还是按消息条数来做判断
	 * </p>
	 * <br>
	 *
	 * @param session
