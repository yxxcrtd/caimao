 ...
 *
 */
public class MinTimeUtil {

    static Logger logger = Logger.getLogger(MinTimeUtil.class.getName());
...
//    }
    ...
    public static String getRedisKeyMultiDaySnapshot(CandleCycle cycle, String exchange,String prodCode) {
    ...
    //格式为交易所代码+产品代码+cycle
    public static String getRedisKey(CandleCycle cycle,String prodCode) {

        StringBuffer sb = new StringBuffer();
        if (!StringUtils.isBlank(prodCode)) {
            sb.append(GJSProductUtils.convertProductSingle(prodCode).get("exchange"));
            sb.append(GJSProductUtils.convertProductSingle(prodCode).get("prodCode"));
            sb.append(cycle);
        }
        return sb.toString();
    }
...
//                    if(!nowTime.equals(miniTime)){
//
//                        logger.error("南交所传入的行情数据日期格式不合法，跟当前日期相差太大:nowTime="+nowTime+",miniTime="+miniTime);
//                        throw new RuntimeException("南交所传入的行情数据日期格式不合法，跟当前日期相差太大:nowTime="+nowTime+",miniTime="+miniTime);
//                    }
                } else {
                    logger.error("南交所传入的行情数据日期格式不合法，长度必须为14位");
                    throw new RuntimeException("南交所传入的行情数据日期格式不合法，长度必须为14位");
...
//        String strBegin0600=time.substring(0,8)+"060000";
...
//        return isTradeTime;
     ...
     * 如果是收盘 就修正下时间，减1毫秒
     * */
    public static String  updateMiniute(String miniTime){
        String resultMiniTime="";
        if(!StringUtils.isBlank(miniTime)&&miniTime.length()==14){
            String strBegin0600=miniTime.substring(0,8)+"060000";
            long begin0600=DateUtils.getTickTime(strBegin0600,"yyyyMMddHHmmss");

            String strEnd0600=miniTime.substring(0,8)+"060100";
            long end0600=DateUtils.getTickTime(strEnd0600,"yyyyMMddHHmmss");


            long source=DateUtils.getTickTime(miniTime,"yyyyMMddHHmmss");
            if(begin0600<=source&&source<=end0600){

                resultMiniTime= DateUtils.addSECOND("yyyyMMddHHmmss", strBegin0600,-1);
            }else{
...
//        MinTimeUtil minTimeUtil = new MinTimeUtil();
...
//        System.out.println(MinTimeUtil.getHour1(baseSnapshot));

        System.out.println(MinTimeUtil.updateMiniute("20151220060000"));
