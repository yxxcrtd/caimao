...
public class JRedisUtil {
    private Logger logger = LoggerFactory.getLogger(JRedisUtil.class);
    @Resource
    public    JedisPool pool ;
    @Resource
    public  JedisPool pool0  ;
    ...
    public String get(int index,String key){
        ...
        String value = null;
        try {
            jedis = pool0.getResource();

            value = jedis.get(key);
        } catch (Exception e) {
            pool0.returnBrokenResource(jedis);
            e.printStackTrace();
        } finally {
            returnResource(pool0, jedis);
        }
        return value;
    }
    /**
     ...
     * @return 成功返回OK 失败和异常返回null
     */
    public String setex(Jedis jedis,String key,String value,int seconds){
...
//        return res;
//    }
    public Long zadd(String key,double score,String member,int second){
        Jedis jedis = null;
        Long res = null;
        try {
            jedis = pool.getResource();
    ...
    public Long zadd(String key,double score,String member,Boolean isRemoveRepeat){
        Jedis jedis = null;
        Long res = null;
        try {
            jedis = pool.getResource();
            if(isRemoveRepeat){
                isExistToDelete(key,score,jedis);
            }
            res = jedis.zadd(key, score, member);
        } catch (Exception e) {
            pool.returnBrokenResource(jedis);
    ...
    public void  isExistToDelete(String redisKeky,double score, Jedis jedis ){

        if(!StringUtils.isBlank(redisKeky)&&score>0&&null!=jedis){

            try{
                Set<String> set= jedis.zrangeByScore(redisKeky, String.valueOf(score), String.valueOf(score));
                if(null!=set&&set.size()>0){
                    jedis.zrem(redisKeky, (String[]) set.toArray(new String[set.size()]));
                }
            }catch (Exception ex){
    ...
    public  long  zrankByScore(String key,long score){
        long  zrank=-1;
        try{
            Set<String> resultStr=zrangeByScore(key, score, score);
            if(null!=resultStr&&resultStr.size()==1){
                for (String str : resultStr) {
                    if(!StringUtils.isBlank(str)){
                        zrank=zrevrank(key,str);
                        break;
                    }
                }
            }
    ...
    public Set<String> zrangeAndDelete(String key ,long start ,long end){
        Jedis jedis = null;
        Set<String> res = null;
        try {
            jedis = pool.getResource();
            ...
            if(null!=res&&res.size()>0){
                jedis.zrem(key, (String[]) res.toArray(new String[res.size()]));
            }
