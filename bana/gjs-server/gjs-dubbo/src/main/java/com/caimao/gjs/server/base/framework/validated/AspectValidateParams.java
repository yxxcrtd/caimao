package com.caimao.gjs.server.base.framework.validated;
...
public class AspectValidateParams {
    ...
    @Pointcut("execution(* com.caimao.gjs.server.service.*.*(..))")
    ...
    public void doBefore(JoinPoint jp) throws Exception {
        //logger.info("切面验证 {} 类中 {} 方法参数 {}", jp.getTarget().getClass(), jp.getSignature().getName(), jp.getArgs());
        if (jp.getArgs().length != 0) { // 有参数
            String errorMsg = null;
            for (int i = 0; i < jp.getArgs().length; i++) { // 验证所有参数
                //logger.info("验证第 {} 个参数值 {}", i, ToStringBuilder.reflectionToString(jp.getArgs()[i]));
                ...
                for (ConstraintViolation<Object> constraintValidator : set) {
                    ...
                    //logger.warn("参数验证 {} 验证错误 : {}", jp.getArgs()[i], constraintValidator.getMessage());
                }
            }
            if (errorMsg != null) {
                // 抛出第一个就可以了
                throw new CustomerException(errorMsg, 999999);
            }
        }
    }
}
